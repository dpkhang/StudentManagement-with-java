...
public class DataModel {
    ...
    public int updateUser(String ten, String email, String phone, String magv ){
        int bool = 0;
        try {
            conn.openDB();
            String query = "update table giangvien set ten = ?, email = ?, phone = ? where magv = ?";
            PreparedStatement ps = conn.conn.prepareStatement(query);
            ps.setString(1, ten);
    ...
    public int deleteStudent(String mssv) {
        ...
        try {
            conn.openDB();
            PreparedStatement ps;
            String query = "detele from sinhvien where mssv = ?";
            ps = conn.conn.prepareStatement(query);
            ps.setString(1, mssv);
            bool = ps.executeUpdate();
        }catch(Exception ex) {
